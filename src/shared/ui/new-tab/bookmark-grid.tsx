...
function AddBookmarkButton({ onClick }: AddBookmarkButtonProps) {
  return (
    ...
    >
      ...
      >
        <Plus className={cn('size-5 text-muted-foreground transition-colors', 'group-hover:text-primary')} />
      </div>

      {/* Label */}
      <span className="text-xs text-muted-foreground transition-colors group-hover:text-foreground">Add</span>
    </button>
  )
}
