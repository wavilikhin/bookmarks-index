...
export const loadAllData = action(async (userId: string) => {
  ...
  try {
    ...
    if (spaces.length === 0) {
      ...
      // Persist seed data
      await Promise.all([setSpaces(userId, spaces), setGroups(userId, groups), setBookmarks(userId, bookmarks)])
    }
...
export const deleteGroup = action(async (userId: string, id: string, hard = false) => {
  ...
  if (hard) {
    ...
    await Promise.all([setGroups(userId, updatedGroups), setBookmarks(userId, updatedBookmarks)])
