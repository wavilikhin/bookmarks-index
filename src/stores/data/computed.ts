 ...
 */
export const sortedSpacesAtom = computed(() => [...spacesAtom()].sort((a, b) => a.order - b.order), 'data.sortedSpaces')

/**
 * Get space by ID
 */
export function getSpaceById(id: string) {
  return computed(() => spacesAtom().find((s) => s.id === id), `data.spaceById.${id}`)
}
